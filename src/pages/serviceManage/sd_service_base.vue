...
<script>
	import { sd_getPlanList, IsTabExpire } from '../../api/api';
	export default {
		data() {
			return {
				planList: [],
				...
				para: {
					...
					strQuery: { Client: '', OverClass: '', ConUnits: '', ProjectName: '', LoginCode: '', CheckState
: 1 },
				},
				listLoading: false
			}
		...
		methods: {
			...
			getList() {
				...
				sd_getPlanList(this.para).then((res) => {
			...
			handleEdit: function (row) {
				IsTabExpire({ProjectId: row.ID}).then(res => {
					if (res.data.type == 1) {
						this.$router.push({ path: 'sd_service_list', query: { sign: row.ID } })
					}
				})	
			}
		},
