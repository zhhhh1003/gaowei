...
<div>
  ...
  <el-button @click='addTask' type="primary" style='margin: 10px;' v-if='false'>添加任务</el-button>
  ...
  <el-table :data="taskList" highlight-current-row v-loading="listLoading" style="width: 100%;" height='430'border>
    ...
    <el-table-column label="操作" width="300" align='center'>
      <template slot-scope="scope">
        <el-button :type="btnColor(scope.row.State)" @click='editRow(scope.row)' size="small"><i class='el-icon-edit'></i>&nbsp{{scope.row.State == 0?'审核':'查看'}}</el-button>
        <el-button type="danger" size="small" @click="handleBack(scope.row)" v-if='scope.row.State == 0'>驳回</el-button>
        <el-button size="small" v-if='scope.row.ReturnTak == 1' @click="rejectRcd(scope.row)">驳回记录</el-button>
        <el-button size="small" @click="handleDel(scope.row)" v-if='false'>删除</el-button>
      </template>
    </el-table-column>
  </el-table>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      addTask () {
        this.$router.push({ path: 'sd_service_info' })
      },
      editRow (val) {
        this.$router.push({ path: 'sd_service_info', query: { ID: val.ID, proName: this.serviceData.ProjectName } })
      },
      handleDel (val) {
