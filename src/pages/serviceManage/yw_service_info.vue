...
<div>
  ...
  <el-table :data="taskList" highlight-current-row v-loading="listLoading" height='390' @selection-change="selsChange" border>
    ...
    <el-table-column width="300" label="业务管理方" align='center'>
      <el-table-column width="150" label="单位" align='center' v-if='false'>
        <template slot-scope="scope">
          <el-input v-show="scope.row.edit" v-model="scope.row.ProUnit"></el-input>
          <span v-show="!scope.row.edit">{{ scope.row.ProUnit }}</span>
        </template>
      </el-table-column>
      <el-table-column width="150" label="负责人" align='center'>
        <template slot-scope="scope">
          <el-input v-show="scope.row.edit" v-model="scope.row.ProChargePerson2"></el-input>
          <span v-show="!scope.row.edit">{{ scope.row.ProChargePerson2 }}</span>
        </template>
      </el-table-column>
    </el-table-column>
...
<script>
  import { getTemplateList, getDetail, getTplInfo, zj_getDetail, zj_editPlan, zj_delDetail, getPermitList, subPermit } from '../../api/api';
  export default {
    data () {
     return {
      existDel: false,
    ...
    methods: {
      ...
      saveTable () {
        ...
        this.submitRow(() => {
          ...
          eval('that.createPara.strMain.State = 2')
            ...
            if (res.data.type == 1) {
              subPermit({data: that.PermitList}).then(res => {
                if (res.data.type == 1) {
                  this.listLoading = false
                  this.canSub = true
                  this.$router.push({ path: 'yw_service_base' })
                }
              })
              }else {}
          })
      ...
      fillTable () {
        zj_getDetail({id: this.$route.query.ID}).then(res => {
          ...
          if (resObj.title.cState != 1 || proCreateBy == '20180123142756983168484001a045f') {
            this.canSub = true
