		...
		<el-table :data="confirmList" highlight-current-row v-loading="listLoading" style="width: 100%;">
				...
				<el-table-column label="操作" width="150" align='center'>
					<template slot-scope="scope">
						<el-button size="small" :type="scope.row.cState == 2?'primary':'warning'" @click="handleEdit(scope.$index, scope.row)" :disabled='scope.row.cState ==-1 || scope.row.cState ==0'>{{btnText(scope.row.cState)}}</el-button>
					</template>
				</el-table-column>
		</el-table>
		<!--工具条-->
	...
	export default {
		...
		methods: {
			...
			handleEdit: function (index, row) {
				this.$router.push({ path: 'yw_confirm_info', query: { sign: row.TaskID } })
			}
		},
		watch: {
