...
<div>
  ...
  <el-table :data="taskList" highlight-current-row v-loading="listLoading" style="width: 100%;" height='430'border>
    ...
    <el-table-column label="操作" width="300" align='center'>
      <template slot-scope="scope">
        <el-button :type="scope.row.State == 3?'primary':'warning'" @click='editRow(scope.row)' size="small" :disabled='!((canReject && scope.row.State == 1) || (!canReject && scope.row.State == 2) || scope.row.State == 3)'><i class='el-icon-edit'></i>&nbsp{{btnText(scope.row)}}</el-button>

        <el-button type="danger" size="small" @click="handleBack(scope.row)" v-if='(canReject && scope.row.State == 1) || (!canReject && scope.row.State == 2)'>驳回</el-button>
        <el-button size="small" v-if='scope.row.ReturnTak == 1' @click="rejectRcd(scope.row)">驳回记录</el-button>
        <el-button v-if='false' size="small" @click="handleDel(scope.row)">删除</el-button>
      </template>
    </el-table-column>
  </el-table>
...
<script>
  ...
  export default {
    ...
    computed: {
      canReject () {
        let id = sessionStorage.getItem("proCreateBy")
        return id == "20180123142756983168484001a045f"
      }
    },
    methods: {
      acrossType (val) {
      ...
      addRow () {
        this.$router.push({ path: 'HSE_service_info' })
      },
      editRow (val) {
        this.$router.push({ path: 'HSE_service_info', query: { ID: val.ID, proName: this.serviceData.ProjectName } })
      },
      handleDel (val) {
      ...
      btnText (val) {
        ...
        }else if(val.State == 3 || val.State == -2){
          return '查看'
        }else if(val.State == -1){
          return '待提交'
        }
