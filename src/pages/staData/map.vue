...
export default {
	...
	methods: {
			centerPoint (e) {
				if (this.$route.query.ID) {
				 	this.$confirm('确定在此点创建吗？', '提示', {}).then(() => {
						...
						let existA = allOverlay.filter(e => {
							return e.ID == this.$route.query.ID
						})
						this.map.removeOverlay(existA[0]);
						this.drawArea(cp, 500, () => {}, {
							tabID: this.$route.query.ID,
						})
					})
				}else {
					return
		    ...
		    drawArea (cp, radius, cb, info) {
				...
				if (info.tabID == this.$route.query.ID) {
					...
					markerMenu.addItem(new BMap.MenuItem('完成',() => {
						c1.disableEditing()
						let fanilCP = new BMap.Point(c1.getCenter().lng, c1.getCenter().lat);
						this.addPara.Rlevel = c1.getRadius()
						this.addPara.TabId = this.$route.query.ID;
						this.addPara.Lng = c1.getCenter().lng;
						this.addPara.Lat = c1.getCenter().lat
						//比较距离
				    	if (this.overlays.length > 0) {
				            for (let j = 0; j < this.overlays.length; j++) {
				            	if (this.overlays[j].ID != this.$route.query.ID) {
				            		if (BMapLib.GeoUtils.getDistance(this.overlays[j].getCenter(), fanilCP) < (this.overlays[j].getRadius() + c1.getRadius())) {
	...
	mounted () {
		...
		that.map = new BMap.Map('map', { minZoom: 13, maxZoom: 14, enableMapClick: false });
	    ...
	    that.map.centerAndZoom(point1, 13);
	    var tileLayer = new BMap.TileLayer({ isTransparentPng: true });
	    tileLayer.getTilesUrl = function (tileCoord, zoom) {
	        var x = tileCoord.x;
	        var y = tileCoord.y;
	        return 'static/tiles/' + zoom + '/title' + x + '_' + y + '.png';
	    }
